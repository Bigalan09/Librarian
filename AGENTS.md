...
- Template variables in rule destinations: `{year}`, `{month}`, `{date}`, `{ext}`, `{source}`, `{ai_suggest}`
- `{ai_suggest}` in a rule destination delegates folder placement to the AI pipeline while preserving the rule's tags/colour
