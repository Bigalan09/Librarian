    ...
    fn all_variants_round_trip_through_append_decision() {
        ...
        let cases: &[(DecisionType, DecisionOutcome, &str)] = &[
            ...
            (
                DecisionType::Move,
                DecisionOutcome::Success,
                "moved to /dest/file.pdf",
            ),
            (
                DecisionType::Rename,
                DecisionOutcome::Success,
                "renamed to 2026-04-17_report.pdf",
            ),
            (
                DecisionType::Tag,
                DecisionOutcome::Success,
            ...
            (
                DecisionType::Correction,
                DecisionOutcome::Corrected,
                "rollback: reversed move",
            ),
            (
                DecisionType::Reorganisation,
                DecisionOutcome::Failed,
                "reorganisation failed",
            ),
            (
                DecisionType::Ignored,
                DecisionOutcome::Skipped,
                "ignored by .librarianignore",
            ),
        ];
        ...
        assert_eq!(
            decisions.len(),
            cases.len(),
            "every appended decision must be readable"
        );

        for (i, (&(dt, outcome, action), decision)) in
            cases.iter().zip(decisions.iter()).enumerate()
        {
            assert_eq!(
                decision.decision_type, dt,
                "row {}: decision_type mismatch",
                i
            );
            assert_eq!(decision.outcome, outcome, "row {}: outcome mismatch", i);
            assert_eq!(decision.action, action, "row {}: action mismatch", i);
            assert_eq!(
                decision.file_hash,
                format!("hash{}", i),
                "row {}: file_hash mismatch",
                i
            );
        }
    }
